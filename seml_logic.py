...
Types:
 ...
 calc      : calculate the actual Fake or Reality of a URL. Use args [API Key], [URL] [Input File] [Calculated Sites**]
Args:
...
def main():
    ...
    f_type = None
    f_type2 = eval("") # default operation of nothing.
    args = {}
    for a in range(1, len(sys.argv)): # for each argument in the sys.argv.
        if skip != 0:
            skip -= 1
            continue
        if sys.argv[a] == "-h" or sys.args[a] == "-help":
            print(help_msg)
            quit()
        if sys.argv[a] == "-t" or sys.argv[a] == "-type": # if the key is typef
            ...
            if sys.argv[a+1] == "calc":
                ...
                args["type"] = sys.argv[a+1]
        if sys.argv[a] == "-key" or sys.argv[a] == "-api-key":
            skip += 1
        ...
        if sys.args[a] == "-p" or sys.args[a] == "-previous":
            skip += 1
    ...
    if args["type"] == "calc":
        ...
        t = f_type(args["api-key"], args["url"], args["input-file"], args["calculated-sites"])
