...
help_msg = """
Usage: python seml_logic.py -t [type] [*args]
Types:
 getpage   : get a pages content and put it in file. Use args [API Key], [URL], [News Type**]
 prob      : find probibility of data after formating file. Use args [Input File] [Output File] [Previous Output**]
 calc      : calculate the actual Fake or Reality of a URL. Use args [API Key], [URL] [Input File] [Calculated Sites**] [Depth**]
Args:
-a  --api-key            : the API key for Monkey Learn's API
-u  --url                : the url of a web page, include the "http://" or "https://"
-n  --news-type          : the type of news for an article, can be "Real" or "Fake"
-i  --input  --inputfile  : the file path for an input file
-o  --output  --outputfile: the file path of an output file
-p  --previous           : the file path for a previous output file generated by the 'prob' function type
-c  --calculated  --calculated-sites : the file path for already calculated sites
-d --depth              : the depth of the algorith, default is 50
...
class SEML:
    ...
    def get_prob(self, filename, outfile, infile=None):
        ...
        # finaly dump all the data to a json file.
...
def main():
    # assign a start time, and end time so we can calculate the time it took to run the program.
    starttime = datetime.datetime.now()
    seml_inst = SEML() # create an instance of the class SEML.
    # variables for type of function and other arguments.
    f_type = None
    f_type2 = eval("0") # default operation of nothing.
    args = {"type": "null", "depth": 50}
    skip = 0
    for a in range(1, len(sys.argv)): # for each argument in the sys.argv.
        if skip != 0:
            skip -= 1
            continue
        if sys.argv[a] == "-h" or sys.argv[a] == "--help":
            print(help_msg)
            quit()
        if sys.argv[a] == "-t" or sys.argv[a] == "--type": # if the key is typef
            skip += 1
            if sys.argv[a+1] == "getpage":
                f_type = seml_inst.get_page
                f_type2 = seml_inst.change_api_key
                args["type"] = sys.argv[a+1]
            if sys.argv[a+1] == "prob":
                f_type = seml_inst.get_prob
                args["type"] = sys.argv[a+1]
            if sys.argv[a+1] == "calc":
                f_type = seml_inst.calculate
                args["type"] = sys.argv[a+1]
        if sys.argv[a] == "-a" or sys.argv[a] == "--key" or sys.argv[a] == "--api-key":
            skip += 1
            args["api-key"] = sys.argv[a+1]
        if sys.argv[a] == "-u" or sys.argv[a] == "--url":
            skip += 1
            args["url"] = sys.argv[a+1]
        if sys.argv[a] == "-n" or sys.argv[a] == "--news-type":
            skip += 1
            args["news-type"] = sys.argv[a+1]
        if sys.argv[a] == "-i" or sys.argv[a] == "--input" or sys.argv[a] == "--input-file":
            skip += 1
            args["input-file"] = sys.argv[a+1]
        if sys.argv[a] == "-o" or sys.argv[a] == "--output" or sys.argv[a] == "--output-file":
            skip += 1
            args["output-file"] = sys.argv[a+1]
        if sys.argv[a] == "-p" or sys.argv[a] == "--previous":
            skip += 1
            args["previous"] = sys.argv[a+1]
        if sys.argv[a] == "-calc" or sys.argv[a] == "--calculated" or sys.argv[a] == "--calculated-sites":
            skip += 1
            args["calculated-sites"] = sys.argv[a+1]
        if sys.argv[a] == "-d" or sys.argv[a] == "--depth":
            skip += 1
            args["depth"] = int(sys.argv[a+1])
    if args["type"] == "getpage":
        if "news-type" not in args:
            args["news-type"] = "FIND"
        f_type2(args["api-key"])
        f_type(args["url"], args["news-type"])
    if args["type"] == "prob":
        if "previous" not in args:
            args["previous"] = None
        f_type(args["input-file"], args["output-file"], args["previous"])
    if args["type"] == "calc":
        if "calculated-sites" not in args:
            args["calculated-sites"] = None
        t = f_type(args["api-key"], args["url"], args["input-file"], args["calculated-sites"], args["depth"])
        console.log("Type of news calculated: {type}".format(type=t))
